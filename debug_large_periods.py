...
class LargePeriodDebugger:
    ...
    async def diagnose_large_period_issue(self, date_from: str = "2025-01-01", date_to: str = "2025-09-24"):
        ...
        try:
            ...
            for api_type in ['wb_sales', 'wb_orders', 'wb_advertising', 'ozon_fbo', 'ozon_fbs']:
                ...
                for i, (chunk_from, chunk_to) in enumerate(chunks):
                    days_in_chunk = (datetime.strptime(chunk_to, '%Y-%m-%d') - datetime.strptime(chunk_from, '%Y-%m-%d')).days
                    logger.debug(f"   Чанк {i+1}: {chunk_from} -> {chunk_to} ({days_in_chunk} дней)")
    ...
    async def _test_wb_sales_direct(self, date_from: str, date_to: str):
        ...
        try:
            ...
            test_chunk_to = (datetime.strptime(date_from, '%Y-%m-%d') + timedelta(days=30)).strftime('%Y-%m-%d')
            ...
            result = {
                ...
                'total_revenue': sum(sale.get('forPay', 0) for sale in sales_data) if sales_data else 0
            }

            logger.debug(f"   ✅ WB Sales результат: {result['sales_count']} продаж, {result['total_revenue']} ₽")
    ...
    def _analyze_root_cause(self):
        ...
        for api_type, data in chunking_data.items():
            if isinstance(data, dict) and data.get('chunk_count', 0) > 20:
                issues_found.append(f"⚠️ Слишком много чанков для {api_type}: {data.get('chunk_count')}")
