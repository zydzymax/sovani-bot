...
class Config:
    ...
    CHATGPT_API_KEY = os.getenv(
        "CHATGPT_API_KEY", None
    )  # Нужно установить через переменную окружения
    ...
    WB_MIN_STOCK_DAYS = int(
        os.getenv("WB_MIN_STOCK_DAYS", "14")
    )  # Минимальное покрытие для WB в днях

    # Настройки Ozon остатков
    OZON_MIN_STOCK_DAYS = int(
        os.getenv("OZON_MIN_STOCK_DAYS", "56")
    )  # Минимальное покрытие для Ozon в днях
    ...
    def print_config_status(cls):
        ...
        print(
            f"  WB Feedbacks Token: {'✅ Настроен' if cls.WB_FEEDBACKS_TOKEN else '❌ Не настроен'}"
        )
        ...
        print(
            f"  WB Analytics Token: {'✅ Настроен' if cls.WB_ANALYTICS_TOKEN else '❌ Не настроен'}"
        )
        print(f"  WB Content Token: {'✅ Настроен' if cls.WB_CONTENT_TOKEN else '❌ Не настроен'}")
        print(
            f"  Ozon API: {'✅ Настроен' if cls.OZON_CLIENT_ID and cls.OZON_API_KEY_ADMIN else '❌ Не настроен'}"
        )
        print(
            f"  Ozon Performance: {'✅ Настроен' if cls.OZON_PERF_CLIENT_ID and cls.OZON_PERF_CLIENT_SECRET else '❌ Не настроен'}"
        )
        print(f"  OpenAI API: {'✅ Настроен' if cls.OPENAI_API_KEY else '❌ Не настроен'}")
        print(
            f"  Manager Chat ID: {'✅ Настроен' if cls.MANAGER_CHAT_ID != 0 else '❌ Не настроен'}"
        )
