...
class PnLCalculator:
    ...
    async def calculate_pnl(
        self, date_from: date, date_to: date, cost_per_unit: float = 0.0
    ) -> PnLData:
        ...
        logger.info(
            f"P&L рассчитан: выручка {pnl.total_revenue:,.0f} ₽, прибыль {pnl.net_profit:,.0f} ₽"
        )
        return pnl
    ...
    async def _calculate_api_expenses(self, pnl: PnLData, date_from: date, date_to: date):
        ...
        try:
            ...
            pnl.other_expenses += ozon_categories.get("returns", 0.0) + ozon_categories.get(
                "other", 0.0
            )
            ...
            pnl.other_expenses += wb_categories.get("returns", 0.0) + wb_categories.get(
                "other", 0.0
            )

            logger.info(
                f"API расходы: комиссии {pnl.marketplace_commissions:,.0f}, реклама {pnl.advertising_costs:,.0f}"
            )
    ...
    async def _calculate_configured_expenses(self, pnl: PnLData):
        ...
        try:
            ...
            units_sold = {"wb": pnl.wb_units, "ozon": pnl.ozon_units, "total": pnl.total_units}
            orders_count = {
            ...
            logger.info(
                f"Настроенные расходы: фиксированные {expense_by_type.get('fixed', 0):,.0f}"
            )
    ...
    async def save_pnl_report(self, pnl: PnLData, filename: str | None = None) -> str:
