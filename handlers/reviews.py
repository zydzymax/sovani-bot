...
def reviews_new_last24():
    ...
    f = (now - timedelta(hours=24)).strftime("%Y-%m-%d")
    j = _get("/reviews", {"date_from": f})
    rows = j.get("rows", [])
    out = [f"<b>Новые отзывы (24ч):</b> {len(rows)}"]
    for r in rows[:10]:
        out.append(
            f"{r.get('platform').upper()} ★{r.get('rating')} {r.get('sku')}: {r.get('text')[:140]}…"
        )
    if len(rows) > 10:
        out.append(f"… и ещё {len(rows)-10}")
    return "\n".join(out)
...
def reviews_list(platform=None, rating_gte=None, rating_lte=None, status=None, limit=10, offset=0):
    ...
    for r in rows:
        out.append(
            f"{r.get('platform').upper()} ★{r.get('rating')} [{r.get('sku')}]: {r.get('text')[:180]}… (id:{r.get('review_id')})"
        )
    return "\n".join(out)
