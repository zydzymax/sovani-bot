...
async def pnl_text(date_from, date_to, group_by="day"):
    j = await _get_async(
        "/live/finance/pnl_v2", {"date_from": date_from, "date_to": date_to, "platform": "all"}
    )
    ...
    if platforms:
        ...
        out.append(
            f"🟦 <b>WB:</b> выручка {money(wb_rev)}, комиссия {money(wb_comm)}, возвраты {money(wb_returns)}"
        )
        ...
        out.append(
            f"🟠 <b>Ozon:</b> выручка {money(ozon_rev)}, комиссия {money(ozon_comm)}, возвраты {money(ozon_returns)}"
        )
