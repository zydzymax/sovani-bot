...
class DataValidator:
        ...
        for i, sale in enumerate(sales):
            ...
            if "forPay" in sale:
                if not isinstance(sale["forPay"], (int, float)):
                    errors.append(
                        f"Sale {i}: forPay должно быть числом, получено {type(sale['forPay'])}"
                    )
                elif sale["forPay"] < 0:
    ...
    def _validate_ozon_orders(
        self, orders: list[dict[str, Any]], order_type: str
    ) -> tuple[bool, list[str]]:
                    ...
                    for j, product in enumerate(order["products"]):
                        if "price" in product:
                            if not isinstance(product["price"], (str, int, float)):
                                errors.append(
                                    f"{order_type} {i} product {j}: price должно быть числом или строкой"
                                )
    ...
    def _cross_validate_data(self, data: dict[str, Any]) -> tuple[bool, list[str]]:
        ...
        if wb_dates and ozon_dates:
            ...
            if abs(wb_min - ozon_min) > timedelta(days=7):
                errors.append(
                    f"⚠️ Большое расхождение в минимальных датах: WB {wb_min}, Ozon {ozon_min}"
                )
...
def validate_data_before_aggregation(data: dict[str, Any]) -> dict[str, Any]:
    ...
    try:
        ...
        if summary["is_valid"]:
            logger.info(
                f"✅ Валидация успешна! WB: {summary['wb_stats']['sales_count']} продаж, Ozon: {summary['ozon_stats']['total_orders']} заказов"
            )
        ...
        return {
            "is_valid": False,
            "errors_count": 1,
            "errors": [f"Критическая ошибка валидации: {e!s}"],
            "validation_timestamp": datetime.now().isoformat(),
        }
