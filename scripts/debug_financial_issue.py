...
async def debug_financial_data():
    ...
    print("=== ОТЛАДКА ФИНАНСОВЫХ ДАННЫХ ===")
    ...
    try:
        ...
        print(
            f"WB результат: revenue={wb_data.get('revenue', 0):,.0f}, units={wb_data.get('units', 0)}"
        )
        ...
        print(
            f"Ozon результат: revenue={ozon_data.get('revenue', 0):,.0f}, units={ozon_data.get('units', 0)}"
        )
