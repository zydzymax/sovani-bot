...
def calculate_year_processing():
    ...
    for days, description in periods:
        ...
        logger.info(f"   Время: {processing_time_minutes:5.1f} мин ({processing_time_hours:4.1f} ч)")
    ...
    logger.info(f"   Дополнительное время на retry: {retry_overhead * DELAY_BETWEEN_REQUESTS / 60:.1f} мин")
    logger.info("")
    ...
    logger.info("")
    logger.info(f"   📅 МАКСИМАЛЬНЫЙ БЕЗОПАСНЫЙ ПЕРИОД: 180 дней (6 месяцев)")
    logger.info(f"      • {results[2]['chunks']} чанков")
    logger.info(f"      • ~{results[2]['time_minutes']:.0f} минут обработки")
    logger.info(f"      • Низкий риск сбоев")
    logger.info("")
    logger.info(f"   📅 ОПТИМАЛЬНЫЙ ПЕРИОД: 90 дней (3 месяца)")
    logger.info(f"      • {results[3]['chunks']} чанков")
    logger.info(f"      • ~{results[3]['time_minutes']:.0f} минут обработки")
    logger.info(f"      • Отличный баланс данных/времени")
    logger.info("")
    logger.info(f"   📅 ДЛЯ ГОДОВЫХ ОТЧЕТОВ:")
    logger.info(f"      • 4 запроса по 90 дней с интервалом")
    logger.info(f"      • Или API + ручные выгрузки")
    logger.info(f"      • Общее время: ~30-40 минут")
