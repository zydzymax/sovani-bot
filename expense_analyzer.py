...
class ExpenseAnalyzer:
        ...
        try:
            ...
            for transaction in transactions:
                ...
                # Комиссии и эквайринг
                    ...
                    word in op_type.lower() for word in ["acquiring", "commission", "marketplace"]
                ):
                ...
                # Логистика
                    ...
                    word in op_type.lower()
                    for word in ["delivery", "logistics", "shipping", "package"]
                ):
                ...
                # Реклама и продвижение
                    ...
                    word in op_type.lower()
                    for word in ["promotion", "advertising", "premium", "top"]
                ):
            ...
            logger.info(
                f"Ozon: проанализировано {len(expenses)} расходов на сумму {total_expenses:,.2f} ₽"
            )
            return result

        except Exception as e:
            logger.error(f"Ошибка анализа расходов Ozon: {e}")
            return {"platform": "ozon", "error": str(e), "total_expenses": 0, "expenses_count": 0}
    ...
    def _get_top_expense_types(self, expenses: list[ExpenseRecord], limit: int = 5) -> list[dict]:
        ...
        sorted_types = sorted(type_totals.items(), key=lambda x: x[1]["amount"], reverse=True)
    ...
    async def get_penalty_alerts(self, days_back: int = 7) -> list[PenaltyAlert]:
        ...
        alerts.sort(
            key=lambda x: ({"high": 3, "medium": 2, "low": 1}[x.severity], x.date), reverse=True
        )
    ...
    async def generate_expense_report(self, date_from: date, date_to: date) -> dict[str, Any]:
        ...
        total_expenses = ozon_analysis.get("total_expenses", 0) + wb_analysis.get(
            "total_expenses", 0
        )
        ...
        report = {
            "period": {"from": date_from.isoformat(), "to": date_to.isoformat()},
            "total_expenses": total_expenses,
            "platforms": {"ozon": ozon_analysis, "wb": wb_analysis},
            "summary": {
    ...
    async def save_expense_report(self, report: dict[str, Any], filename: str | None = None):
...
async def main():
    ...
    print(
        f"  Всего: {penalties['penalties_total']:,.2f} ₽ ({penalties['penalties_count']} штрафов)"
    )
