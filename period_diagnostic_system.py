...
class PeriodDiagnosticSystem:
    ...
    def get_test_periods(self) -> List[Tuple[str, str, str, int]]:
        ...
        periods = [
            ...
            ("1_day", (today - timedelta(days=1)).strftime('%Y-%m-%d'), today.strftime('%Y-%m-%d'), 1),
            ("7_days", (today - timedelta(days=7)).strftime('%Y-%m-%d'), today.strftime('%Y-%m-%d'), 7),
            ("14_days", (today - timedelta(days=14)).strftime('%Y-%m-%d'), today.strftime('%Y-%m-%d'), 14),
            ("30_days", (today - timedelta(days=30)).strftime('%Y-%m-%d'), today.strftime('%Y-%m-%d'), 30),
            ("60_days", (today - timedelta(days=60)).strftime('%Y-%m-%d'), today.strftime('%Y-%m-%d'), 60),
            ("90_days", (today - timedelta(days=90)).strftime('%Y-%m-%d'), today.strftime('%Y-%m-%d'), 90),
        ]
    ...
    async def test_single_period(self, name: str, date_from: str, date_to: str, days: int) -> Dict:
        ...
        try:
            ...
            try:
                ...
                logger.info(f"   ✅ Оригинальные данные получены: {original_data.get('revenue', 0):,.0f} ₽")
            ...
            try:
                corrected_data = await self.corrected_reports.get_corrected_wb_data(date_from, date_to)
                test_result['corrected_data'] = corrected_data
                logger.info(f"   ✅ Исправленные данные получены: {corrected_data.get('revenue', 0):,.0f} ₽")
            except Exception as e:
            ...
            if original_data and corrected_data:
                ...
                logger.info(f"   🔄 Соотношение оригинал/исправленный: {analysis.get('correction_ratio', 0):.2f}x")
                logger.info(f"   📈 Выручка в день (оригинал): {analysis.get('daily_revenue_original', 0):,.0f} ₽")
                logger.info(f"   📈 Выручка в день (исправленная): {analysis.get('daily_revenue_corrected', 0):,.0f} ₽")
    ...
    def analyze_period_results(self, original_data: Dict, corrected_data: Dict, days: int) -> Dict:
        ...
        analysis = {
            ...
            'correction_ratio': original_revenue / corrected_revenue if corrected_revenue > 0 else 0,
            ...
            'revenue_difference_pct': ((original_revenue - corrected_revenue) / corrected_revenue * 100) if corrected_revenue > 0 else 0
        }
    ...
    def print_summary(self, results: Dict):
        ...
        for period_name, result in results.items():
            if result.get('success'):
                ...
                logger.info(f"{period_name:>10}: {status} | {revenue_orig:>10,.0f} → {revenue_corr:>8,.0f} ₽ ({ratio:.1f}x)")
            else:
                status = "❌ ОШИБКА"
