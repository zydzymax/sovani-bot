...
class RateLimiter:
    ...
    async def wait_for_rate_limit(self, api_name: str) -> None:
        ...
        self.request_counts[api_name] = [
            t for t in self.request_counts[api_name] if t > minute_ago
        ]
        ...
        if len(self.request_counts[api_name]) >= config.requests_per_minute:
            ...
            if wait_time > 0:
                logger.info(f"{api_name}: Достигнут лимит {config.requests_per_minute} req/min, ждем {wait_time:.1f}с")
        ...
        if self.burst_tokens[api_name] <= 0:
            ...
            if time_since_last >= (config.min_interval_ms / 1000):
                ...
                tokens_to_restore = min(config.burst_limit, int(time_since_last / (config.min_interval_ms / 1000)))
    ...
    async def handle_429_error(self, api_name: str, attempt: int = 0) -> None:
        ...
        logger.error(f"🚨 {api_name}: 429 Too Many Requests, экспоненциальная задержка {wait_time}с (попытка {attempt + 1}, множитель {exponential_multiplier}x)")
...
def get_rate_limit_stats() -> Dict:
    ...
    return {
        api_name: rate_limiter.get_api_stats(api_name)
        for api_name in rate_limiter.CONFIGS.keys()
    }