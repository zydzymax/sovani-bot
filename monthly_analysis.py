    ...
    def analyze_discrepancies(self):
        ...
        for month_data in monthly_breakdown:
            if 'error' not in month_data:
                ...
                logger.info(f"   {month_data['month']}: {month_data['revenue']:,.0f} ₽ (соотношение: {month_avg:.1f}x)")
        ...
        logger.info(f"   📊 Используется: priceWithDisc (цена со скидками)")
        logger.info(f"   🎯 Источник: Sales API (реальные выкупы)")

        if total_orders > total_sales * 2:
            logger.info(f"   ⚠️ МНОГО ЗАКАЗОВ vs ПРОДАЖ - возможно учитываются невыкупленные заказы")

        # 2. Период анализа
        logger.info(f"2️⃣ ПЕРИОД АНАЛИЗА:")
        logger.info(f"   📅 Системный: 01.01.2025 - 26.09.2025 (268 дней)")
        logger.info(f"   ❓ Пользователь ожидал: возможно другой период?")

        # 3. Платформы
        logger.info(f"3️⃣ ОХВАТ ПЛАТФОРМ:")
        logger.info(f"   🟣 Система: только Wildberries")
        logger.info(f"   ❓ Пользователь ожидал: только WB или WB+Ozon?")

        # 4. Типы операций
        logger.info(f"4️⃣ ТИПЫ ОПЕРАЦИЙ:")
        logger.info(f"   ✅ Система учитывает: isRealization = true")
        logger.info(f"   📊 Базовая цена: priceWithDisc (после скидки продавца)")
        logger.info(f"   ❓ Пользователь ожидал: возможно другую методику?")
