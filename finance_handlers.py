...
def preset_range(preset: str):
    ...
    if preset == "today":
        ...
        t = now; f = now - dt.timedelta(days=7)
...
async def report_pnl(date_from, date_to, group_by="day"):
    data = await _get_async("/finance/pnl", params={"date_from":date_from,"date_to":date_to,"group_by":group_by})
    rows = data.get("rows", [])
    total = {
    ...
    lines.append(
        "Выручка: <b>{}</b>\nВозвраты: {}\nКомиссия МП: {}\nЛогистика: {}\nCOGS: {}\nРеклама: {}\nOPEX: {}\n<b>Валовая прибыль:</b> {}\n<b>Чистая прибыль:</b> {}".format(
            fmt_money(total['revenue']), fmt_money(total['returns']),
            fmt_money(total['mp_commission']), fmt_money(total['logistics']),
            fmt_money(total['cogs']), fmt_money(total['ads']), fmt_money(total['opex']),
            fmt_money(total['gross_profit']), fmt_money(total['net_profit'])
        )
    )
    if group_by=="day" and rows:
        lines.append("\n<b>По дням:</b>")
        for r in rows[-14:]:  # последние 14 строк, чтобы влезало
            lines.append("{} — выручка {}, рекл {}, чистая {}".format(
                r.get("key"), fmt_money(r.get("revenue",0)), fmt_money(r.get("ads",0)), fmt_money(r.get("net_profit",0))
            ))
    return "\n".join(lines)
...
async def report_dds(date_from, date_to):
    ...
    lines.append("Поступления: <b>{}</b>\nСписания: {}\n<b>Чистый поток:</b> {}".format(
        fmt_money(total_in), fmt_money(total_out), fmt_money(net)
    ))
    # агрегируем по типам если есть поле type
    agg = {}
    for r in rows:
        t = r.get("type","прочее"); agg.setdefault(t, {"in":0,"out":0})
        agg[t]["in"] += r.get("in",0); agg[t]["out"] += r.get("out",0)
    if agg:
        lines.append("\n<b>По категориям:</b>")
...
async def report_romi(date_from, date_to):
    ...
    lines.append("Затраты на рекламу: {}\nПрибыль, атрибутированная рекламе: {}\n<b>ROMI:</b> {:.2f}".format(
        fmt_money(total_cost), fmt_money(total_profit), romi
    ))
    # Топ кампаний
    ...
    if rows:
        ...
        for r in rows:
            key = r.get("key","-")
            lines.append(f"{key}: ROMI {r.get('romi',0):.2f}, cost {fmt_money(r.get('ads_cost',0))}, profit {fmt_money(r.get('attributed_profit',0))}")
    return "\n".join(lines)