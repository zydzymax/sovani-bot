...
class ReportAnalyzer:
    ...
    def load_wb_stock_json(self, file_path: str) -> pd.DataFrame | None:
        ...
        try:
            ...
            if "available_stock" in df.columns:
                df["available_stock"] = pd.to_numeric(
                    df["available_stock"], errors="coerce"
                ).fillna(0)
    ...
    def load_wb_sales_report(self, file_path: str) -> pd.DataFrame | None:
    ...
    def load_wb_stock_report(self, file_path: str) -> pd.DataFrame | None:
    ...
    def load_ozon_sales_report(self, file_path: str) -> pd.DataFrame | None:
    ...
    def load_ozon_stock_report(self, file_path: str) -> pd.DataFrame | None:
    ...
    def calculate_pnl(
        self, wb_sales_df: pd.DataFrame, ozon_sales_df: pd.DataFrame
    ) -> dict[str, Any]:
        ...
        try:
            ...
            if wb_sales_df is not None and not wb_sales_df.empty:
                ...
                if "to_pay" in wb_sales_df.columns:
                    ...
                    pnl_data["wb"]["revenue"] = (
                        wb_sales_df["price"] * wb_sales_df["quantity"]
                    ).sum()
                ...
                if "sku" in wb_sales_df.columns:
                    ...
                    if "to_pay" in wb_grouped.columns:
                        wb_grouped["profit"] = wb_grouped["to_pay"] - (
                            wb_grouped["quantity"] * self.cost_price
                        )
            ...
            if ozon_sales_df is not None and not ozon_sales_df.empty:
                ...
                if "sku" in ozon_sales_df.columns:
                    ozon_grouped = (
                        ozon_sales_df.groupby("sku")
                        .agg({"quantity": "sum", "revenue": "sum"})
                        .reset_index()
                    )

                    ozon_grouped["profit"] = ozon_grouped["revenue"] - (
                        ozon_grouped["quantity"] * self.cost_price
                    )
            ...
            if pnl_data["total"]["revenue"] > 0:
                pnl_data["total"]["margin_percent"] = (
                    pnl_data["total"]["profit"] / pnl_data["total"]["revenue"]
                ) * 100
    ...
    def calculate_replenishment(
        self,
        wb_stock_df: pd.DataFrame,
        wb_sales_df: pd.DataFrame,
        ozon_stock_df: pd.DataFrame,
        ozon_sales_df: pd.DataFrame,
    ) -> list[dict[str, Any]]:
        ...
        try:
            # Анализ Wildberries
            if (
                wb_stock_df is not None
                and wb_sales_df is not None
                and not wb_stock_df.empty
                and not wb_sales_df.empty
            ):
                # Рассчитываем средние продажи по дням для каждого SKU+размер
                wb_sales_daily = (
                    wb_sales_df.groupby(["sku", "size"])["quantity"].sum().reset_index()
                )
                ...
                wb_analysis = wb_stock_df.merge(
                    wb_sales_daily[["sku", "size", "daily_sales"]], on=["sku", "size"], how="left"
                )
                ...
                wb_analysis["cover_days"] = wb_analysis.apply(
                    lambda row: (
                        row["stock"] / row["daily_sales"] if row["daily_sales"] > 0 else 999
                    ),
                    axis=1,
                )
            ...
            # Анализ Ozon
            if (
                ozon_stock_df is not None
                and ozon_sales_df is not None
                and not ozon_stock_df.empty
                and not ozon_sales_df.empty
            ):
                ...
                ozon_analysis = ozon_stock_df.merge(
                    ozon_sales_daily[["sku", "daily_sales"]], on="sku", how="left"
                )
        ...
        try:
            ...
            if campaigns:
                ...
                if campaigns_by_type:
                    parts.append("")
                    parts.append("📊 <b>Детализация кампаний:</b>")
...
⏰ <b>Автоматическая проверка:</b> ежедневно в 06:00"""
        ...
        return f"❌ Критическая ошибка при генерации отчета: {e!s}"
...
⏰ <b>Автоматическая проверка:</b> ежедневно в 06:00"""
        ...
        return f"❌ Ошибка при генерации отчета: {e!s}"
